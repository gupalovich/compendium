...
from typing import TYPE_CHECKING, Any, ClassVar, Final, Literal, Protocol, TypeVar
...
_MOUSEEVENTF_XCLICK: Final = _MOUSEEVENTF_XDOWN + _MOUSEEVENTF_XUP  # c_ulong(0x0180)
...
def _create_keyboard_input(
    ...
    input_struct.ki = _KEYBDINPUT(wVk, wScan, dwFlags, time, pointer(dwExtraInfo))
    return input_struct
...
def _create_hardware_input(  # pyright: ignore[reportUnusedFunction]
    uMsg: int = 0, wParamL: int = 0, wParamH: int = 0  # c_ulong  # c_short  # c_ushort
) -> _INPUT:
...
class _MapVirtualKeyWType(Protocol):
    ...
    def __call__(self, uCode: c_uint | int, uMapType: c_uint | int) -> int:  # c_uint
        ...
...
class _GetCursorPosType(Protocol):
    ...
    def __call__(self, lpPoint: _POINTER_TYPE[_POINT] | _POINT) -> bool:  # c_bool
        ...
...
def _calc_normalized_screen_coord(pixel_coord: int, display_total: int) -> int:
    ...
    this_pixel: Final[int] = (pixel_coord * 65536 + display_total - 1) // display_total
...
def _to_windows_coordinates(
    ...
    windows_x: int = _calc_normalized_screen_coord(x - offset_left, display_width)
    windows_y: int = _calc_normalized_screen_coord(y - offset_top, display_height)
...
def position(
    ...
    return (cursor.x if x is None else int(x), cursor.y if y is None else int(y))
...
def size() -> tuple[int, int]:
    ...
    """
    return (_get_system_metrics(_SM_CXSCREEN), _get_system_metrics(_SM_CYSCREEN))
...
def hscroll(
    ...
    *,
    ...
    for _ in range(clicks):
        ...
        _send_input(
            _create_mouse_input(
                mouseData=(direction * _WHEEL_DELTA), dwFlags=_MOUSEEVENTF_HWHEEL
            )
        )
    ...
    *,
    ...
    for scancode in scancodes_sequence:
        if auto_shift and scancode & _OFFSET_SHIFTKEY:
            input_structs += [
                _create_keyboard_input(wScan=_SHIFT_SCANCODE, dwFlags=keybdFlags)
            ]
            expectedEvents += 1
        ...
        input_structs += [
            _create_keyboard_input(wScan=scancode, dwFlags=keybdFlags | extendedFlag)
        ]
        expectedEvents += 1
...
def _helper_scancode_press(
    ...
    """
    downed: bool = scancode_keyDown(scancodes, _pause=_pause, auto_shift=auto_shift)
    _sleep(duration)
    upped: bool = scancode_keyUp(scancodes, _pause=_pause, auto_shift=auto_shift)
...
def keyUp(
    ...
    *,
    ...
    if scancode is None:
        return False
    return scancode_keyUp(scancode, logScreenshot, _pause=_pause, auto_shift=auto_shift)
...
# ----- _helper_press ----------------------------------------------------------
def _helper_press(
    key: str, duration: float = 0.0, _pause: bool = True, auto_shift: bool = False
) -> bool:
    """
...
def typewrite(
    ...
    *,
    ...
    for key in message:
        ...
        press(key, _pause=False, auto_shift=auto_shift, delay=delay, duration=duration)
...
def unicode_charDown(
    ...
    codes: Sequence[int] = unpack(f">{len(utf16surrogates)//2}H", utf16surrogates)
